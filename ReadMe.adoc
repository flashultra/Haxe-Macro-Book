...
``$a{}``,``Array<Expr>\->Array<Expr> または Array<Expr>\->Expr``,``Array<Expr>`` is written at the expected position, the value is expanded at that position. At the position where ``Expr`` is expected, it is converted into an array declaration expression and expanded.
