...
image::compile.png[Compile process]
...
|======
,type,description
Evaluate the contents of ``${}``, ``$e{}``, ``Expr\->Expr``, ``{}`` and expand to that position
If you write ``$a{}``,``Array<Expr>\->Array<Expr> or Array<Expr>\->Expr``,``Array<Expr>`` in the expected position, Expand the value to its position. At the position where ``Expr`` is expected, it is converted into an array declaration expression and expanded.
``$b{}``,``Array<Expr>\->Expr``, block expression.
Generate identifier from ``$i{}``,``String\->Expr``,string.
...
``function $name () {}``,``String\->Expr``, function declaration.
``{ $name : 1 }``,``String\->Expr``, object literal.
``try e() catch($name:Dynamic) {}``,``String\->Expr``,try-catch
``new $typePath()``,``TypePath\->Expr``, instantiation.
A tag that takes ``@:pos(p)`` and ``Position`` as an argument, replace the position information of the expression with `p`.
|======
...
image::completion.png[Completion in FlashDevelop]
